...
/* function that receives matrix and prints it */
...
/* function that receives matrix and fills it
 with values that the User inserts accordingly  */
		void build_matrix(int matrix[N][N] , int matrix_size )
		{
		     print_enter_matrix();
...
/* function that substitutes each entry In another value
on the basis of the box-s, which is a fixed
 and predefined table, so that the element
 whose value is 'k' will be replaced
In the k-place  */
       void Substitution_Byte(int matrix_size ,int matrix[N][N] )
       {
           for (int i = 0; i<matrix_size ; i++)
         {
           for(int j=0 ; j<matrix_size ; j++)
...
/* the function moves the i line,i places to the left in a circular
 form and inserts the values in a defined
 temporarily matrix , when the loop is over it inserts the values
  of the temporarily matrix into the current matrix.
 */
     void Shift_Rows(int matrix_size ,int matrix[N][N])
     {
      int tmp_matrix[matrix_size];
      ...
      for (int i = 0; i<matrix_size ; i++)
       {
           ...
           {
               int place=i+j ;
               if(place>=matrix_size){place=place-(matrix_size);}

               tmp_matrix[j]= matrix[i][place];

           }
           for(int k=0 ; k<matrix_size ; k++)
            {
                matrix[i][k]= tmp_matrix[k];
            }
       }
...
/* the main function defines values that will contain a
 positive number of iterations and matrix size ,
 it calls the functions the will build the matrix
  and change it (iterations) number of times
   and then print the new matrix
 */
int main()
{
    int iteration =-1 , matrix_size=0;
   ...
   while(iteration>0)
{
    Substitution_Byte(matrix_size , matrix );
    Shift_Rows(matrix_size , matrix);
        iteration-- ;
}
